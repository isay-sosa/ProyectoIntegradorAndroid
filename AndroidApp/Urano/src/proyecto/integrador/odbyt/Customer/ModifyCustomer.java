...
public class ModifyCustomer extends Activity implements OnClickListener {
	...
	private void initComponents() {
		...
		lblHeaderTitle
				.setText(getString(R.string.title_activity_modify_customer));
	...
	private void btnPositive_onClick() {
		if (txtName.getText().toString().trim().length() > 0
			...
			if (customer.updateCustomerById(c.getId(), c.getIdServer(), txtName
					.getText().toString().trim(), txtLastName.getText()
					.toString().trim(), txtAddress.getText().toString().trim(),
					txtCity.getText().toString().trim(), txtState.getText()
							.toString().trim(), txtPhone.getText().toString()
							.trim(), txtCellphone.getText().toString().trim(),
					txtEmail.getText().toString().trim(), true)) {
				...
				// Error occurred.
				Toast.makeText(this, getString(R.string.error_modify_customer),
